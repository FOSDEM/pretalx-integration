...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with scope(event=event):
            tracks = Track.objects.filter(event=event).select_related("tracksettings")\
        ...
        for track in tracks:
            try:
                with scopes_disabled():
                    managers = list(track.trackmanager_set.all().values_list("user__email", flat=True))
                print(f"{track.tracksettings.mail}\t{','.join(managers)},devrooms@fosdem.org")
